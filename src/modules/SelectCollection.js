...
class Select extends BaseComponent {
  ...
  onClick = (event) => {
    ...
    const isButtonClicked = this.buttonElement === target
    const isOutsideDropdownClicked = target.closest(this.selectors.dropdown) !== this.dropdownElement

    if (!isButtonClicked && isOutsideDropdownClicked) {
      this.collapse()
      return
    }

    const isOptionClicked = target.matches(this.selectors.option)
